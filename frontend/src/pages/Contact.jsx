import PageHero from "../components/PageHero";
import "./Contact.css";

const floatingIcons = [
  { icon: "✈️", x: "6%",  y: "16%", size: "2.6rem", dur: "6s",   delay: "0s"   },
  { icon: "🌍", x: "87%", y: "10%", size: "3rem",   dur: "8s",   delay: "1.2s" },
  { icon: "🏔️", x: "4%",  y: "54%", size: "2.2rem", dur: "7s",   delay: "2s"   },
  { icon: "🌴", x: "91%", y: "58%", size: "2.8rem", dur: "9s",   delay: "0.6s" },
  { icon: "🧳", x: "14%", y: "76%", size: "2rem",   dur: "6.5s", delay: "3s"   },
  { icon: "🌊", x: "76%", y: "78%", size: "2.4rem", dur: "7.5s", delay: "1.8s" },
  { icon: "🌅", x: "48%", y: "6%",  size: "2rem",   dur: "8.5s", delay: "2.5s" },
  { icon: "🗺️", x: "34%", y: "84%", size: "2.1rem", dur: "6s",   delay: "4s"   },
  { icon: "📸", x: "69%", y: "22%", size: "2rem",   dur: "7s",   delay: "3.5s" },
  { icon: "🎒", x: "24%", y: "13%", size: "2.3rem", dur: "9s",   delay: "2.2s" },
  { icon: "⛵", x: "59%", y: "73%", size: "2rem",   dur: "7s",   delay: "1s"   },
  { icon: "🦋", x: "81%", y: "43%", size: "1.8rem", dur: "6s",   delay: "0.3s" },
];

const Contact = () => {
  return (
    <div className="contact-page">
      {/* خلفية متحركة */}
      <div className="contact-bg" aria-hidden="true">
        <div className="blob blob-1" />
        <div className="blob blob-2" />
        <div className="blob blob-3" />
        {floatingIcons.map((item, i) => (
          <span
            key={i}
            className="floating-icon"
            style={{
              left: item.x,
              top: item.y,
              fontSize: item.size,
              animationDuration: item.dur,
              animationDelay: item.delay,
            }}
          >
            {item.icon}
          </span>
        ))}
      </div>

      <PageHero title="تواصل معنا" subtitle="كل رحلة تبدأ بسؤال — لا تتردد" icon="💬" waveColor="rgba(240, 235, 227, 0.78)" />

      {/* المحتوى */}
      <div className="container contact-body">

        {/* اقتباس */}
        <div className="contact-quote">
          <span className="quote-mark">"</span>
          <p>السفر يُغيّر ما بداخلك قبل أن يُغيّر ما حولك</p>
          <span className="quote-mark quote-mark--end">"</span>
        </div>

        <p className="contact-intro">
          هل عندك سؤال عن رحلة؟ أو بدك تعرف أكثر عن رُحى؟<br />
          تواصل معنا عبر أي قناة وسنرد عليك بأسرع وقت ممكن.
        </p>

        {/* كروت التواصل */}
        <div className="contact-cards">

          {/* واتساب */}
          <a
            href="https://wa.me/972504933918?text=مرحباً رُحى، أريد الاستفسار عن الرحلات"
            target="_blank"
            rel="noreferrer"
            className="contact-card contact-card--whatsapp"

          >
            <div className="contact-card-icon">
              <svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
            </div>
            <div className="contact-card-body">
              <h3>واتساب</h3>
                          <p dir="ltr" >0504933918</p>
              <span>اضغط للمحادثة مباشرة</span>
            </div>
          </a>

          {/* إنستغرام */}
          <a
            href="https://www.instagram.com/ruha_journey"
            target="_blank"
            rel="noreferrer"
            className="contact-card contact-card--instagram"
          >
            <div className="contact-card-icon">
              <svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
              </svg>
            </div>
            <div className="contact-card-body">
              <h3>إنستغرام</h3>
              <p dir="ltr">@ruha_journey</p>
              <span>تابعنا على إنستغرام</span>
            </div>
          </a>

          {/* تيك توك */}
          <a
            href="https://www.tiktok.com/@ruha_journey"
            target="_blank"
            rel="noreferrer"
            className="contact-card contact-card--tiktok"
          >
            <div className="contact-card-icon">
              <svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.34 6.34 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.18 8.18 0 004.78 1.52V6.77a4.85 4.85 0 01-1.01-.08z"/>
              </svg>
            </div>
            <div className="contact-card-body">
              <h3>تيك توك</h3>
              <p dir="ltr">@ruha_journey</p>
              <span>شاهد مقاطعنا</span>
            </div>
          </a>

          {/* إيميل */}
          <a
            href="mailto:ruha@gmail.com"
            className="contact-card contact-card--email"
          >
            <div className="contact-card-icon">
              <svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/>
              </svg>
            </div>
            <div className="contact-card-body">
              <h3>البريد الإلكتروني</h3>
              <p dir="ltr">ruha@gmail.com</p>
              <span>راسلنا بالإيميل</span>
            </div>
          </a>

        </div>

        <div className="contact-note">
          <p>⏱ نرد عادةً خلال 24 ساعة</p>
        </div>
      </div>
      <div className="contact-footer-pad" />
    </div>
  );
};

export default Contact;
